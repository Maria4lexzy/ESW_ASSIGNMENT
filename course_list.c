...
p_course_list_node head_course = NULL;
p_course_list_node current_course = NULL;
uint16_t number_of_nodes_course = 0;
uint16_t list_size_course = 0;

void printCourseList() {

	p_course_list_node tempP = (p_course_list_node)malloc(sizeof(course_list_t));
	tempP = head_course;
	printf("The list contains: (");
	int temp_no_of_nodes = number_of_nodes_course;
	while (tempP && temp_no_of_nodes > 0)
	{
		pcourse_t course_value = (p_course_list_node)tempP->el_course;
		printf("[%d \t %s\t %d]", course_value->course_num, course_value->course_name, course_value->semester_num);
		tempP = (p_course_list_node)(tempP->next_course);	//shifting to the next node in the linked list
		temp_no_of_nodes--;
	}
	free(tempP);
	tempP = NULL;
...
pcourse_t get_course_by_index(uint16_t index)
{
	...
	{
		p_course_list_node headTemp = head_course;				// starting from first node
		if (index == 0)
			return headTemp->el_course;
		else
			for (uint16_t i = 0; i < index; i++)
				headTemp = (p_course_list_node)(headTemp->next_course);	// moving to next node
...
pcourse_t get_course_by_course_number(pcourse_t course_value)
{
	if (number_of_nodes_course >= 0 && course_value != NULL)
	{
		...
		for (uint16_t i = 0; i < number_of_nodes_course; i++)
		{
			...
			headTemp = (p_course_list_node)headTemp->next_course;
		}
	}
	return NULL;
}
...
{
	p_course_list_node new_node = (p_course_list_node)malloc(sizeof(course_list_t));
	...
	else if (number_of_nodes_course < list_size_course && new_node)
	{
		...
		current_course = (p_course_list_node)current_course->next_course;
...
{
	p_course_list_node new_node = (p_course_list_node)malloc(sizeof(course_list_t));
	p_course_list_node headTemp = head_course;
	if (course_value == NULL)
		return -1;
	...
	if (headTemp != NULL)
	{
		p_course_list_node tmp = headTemp->next_course;
...
int remove_first_course(p_course_list_node headValue)
{
	if (headValue == NULL)
		return NULL;
	p_course_list_node front = headValue;
...
int remove_last_course(p_course_list_node headValue)
{
	if (headValue == NULL)
		return -1;
	p_course_list_node cursor = headValue;
	p_course_list_node back = NULL;
